...
function getRectangleArea(width, height) {
  const areaTriangle = width * height;
  return areaTriangle;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const current = Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(x2 * x2 + y2 * y2);
  const result = Math.acos(top / current);
  return result;
}
